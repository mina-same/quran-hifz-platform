...
> Auto-maintained by OpenWolf. Last scanned: 2026-09-16T20:58:03.268Z
...
- `login_teacher.mjs` — Declares browser (~312 tok)
- `login_teacher2.mjs` — Declares browser (~305 tok)
- `shot_devtest.mjs` — Declares browser (~187 tok)
...
- `test-slicing.ts` — Case 1: range spanning exactly 10 pages, over 5 days (2 pages/day) — should divide cleanly. (~584 tok)
- `verify.mjs` — BASE: shot (~619 tok)
...
- `driver.mjs` — Declares BASE (~279 tok)

## ../../../../../private/tmp/claude-501/-Users-xontel-Downloads-mina-work-quran-hifz-platform/444e5ae5-663a-4c25-a500-20d8479831d8/scratchpad/pw2/

- `driver.mjs` — BASE: loginAndCheck (~517 tok)
- `driver2.mjs` — Declares BASE (~486 tok)
- `driver3.mjs` — BASE: handler (~375 tok)

## ../../../../../private/tmp/claude-501/-Users-xontel-Downloads-mina-work-quran-hifz-platform/444e5ae5-663a-4c25-a500-20d8479831d8/scratchpad/pw3/

- `verify.mjs` — Declares BASE (~279 tok)

## ../../../../../private/tmp/claude-501/-Users-xontel-Downloads-mina-work-quran-hifz-platform/444e5ae5-663a-4c25-a500-20d8479831d8/scratchpad/pw4/

- `driver.mjs` — Declares BASE (~798 tok)

## ../../../../../private/tmp/claude-501/-Users-xontel-Downloads-mina-work-quran-hifz-platform/444e5ae5-663a-4c25-a500-20d8479831d8/scratchpad/pw5/

- `driver.mjs` — Declares BASE (~784 tok)
- `driver2.mjs` — Declares BASE (~599 tok)
...
## ../../../../../private/tmp/claude-501/-Users-xontel-Downloads-mina-work-quran-hifz-platform/acf85922-ed88-4311-85f5-cdc043cf61d7/scratchpad/

- `driver.mjs` — Declares BASE (~816 tok)

## ../../../../../private/tmp/claude-501/-Users-xontel-Downloads-mina-work-quran-hifz-platform/b9799155-a8b0-4166-8993-e6f0a4d10143/scratchpad/

- `add_bug405.py` (~1092 tok)
- `diagnose_orphans.ts` — Declares main (~578 tok)
...
- `shot_parents.mjs` — Declares OUT (~337 tok)
...
## ../../../../../private/tmp/claude-501/-Users-xontel-Downloads-mina-work-quran-hifz-platform/e47eeed4-65f5-4f15-b91a-f98b52331c6e/scratchpad/pw-verify/

- `driver.mjs` — Declares browser (~639 tok)
...
## .claude/worktrees/phase3-mobile/quran-hifz-mobile/app/

- `index.tsx` — getTrackName — renders form, modal (~3637 tok)

## .claude/worktrees/phase3-mobile/quran-hifz-mobile/app/(portal)/admin/

- `_layout.tsx` — MORE_IDS — renders form (~1134 tok)
...
## .claude/worktrees/phase3-mobile/quran-hifz-mobile/app/(portal)/parent/

- `dashboard.tsx` — ParentDashboard (~1712 tok)

## .claude/worktrees/phase3-mobile/quran-hifz-mobile/app/(portal)/student/

- `_layout.tsx` — MORE_IDS (~922 tok)
- `dashboard.tsx` — getName (~2404 tok)
- `homework.tsx` — getTitle (~1112 tok)
- `schedule.tsx` — DAYS (~2280 tok)

## .claude/worktrees/phase3-mobile/quran-hifz-mobile/app/(portal)/teacher/

- `_layout.tsx` — MORE_IDS (~1065 tok)
...
- `TrackDetail.tsx` — First letter of the first two words — the same initials the web chips show. (~6732 tok)
- `TrackStudentsPanel.tsx` — Track roster management — transfer-only, since a student's track is (~2169 tok)
...
## .claude/worktrees/phase3-mobile/quran-hifz-mobile/lib/constants/

- `portals.ts` — Exports PORTALS, PORTAL_ROUTES (~2299 tok)
...
- `tracks.ts` — Computed server-side (`Student.countDocuments({track})`) — present on (~1134 tok)

## .claude/worktrees/phase3-mobile/quran-hifz-server/src/

- `_verify_accounts.ts` — Declares main (~284 tok)

## .claude/worktrees/phase3-mobile/quran-hifz-server/src/controllers/

- `track.controller.ts` — Moves a student INTO this track — sets their `track` field, replacing (~2230 tok)

## .claude/worktrees/phase3-mobile/quran-hifz-server/src/routes/
...
- `progress.md` — SDD ledger — plan: docs/superpowers/plans/2026-09-05-halqa-track-restructure-phase1-server.md (~4572 tok)
...
- `masajid.tsx` — AdminMasajid (~1245 tok)
...
- `students.tsx` — المسار: real track lives one hop away via `Student.track`, not the unused legacy `path` enum. (~3403 tok)
- `teachers.tsx` — ratingVariant (~1855 tok)
- `track-detail.tsx` — AdminTrackDetailRoute (~474 tok)
- `tracks.tsx` — First letter of the first two words — the same initials the web cards show. (~11743 tok)

## quran-hifz-mobile/app/(portal)/parent/

- `_layout.tsx` — ParentTabLayout (~445 tok)
- `attendance.tsx` — ParentAttendance, real data (useChildAttendance) + pull-to-refresh + SkeletonRows (~960 tok)
- `dashboard.tsx` — ParentDashboard, real data (useParentChildren/useChildHifz/useChildMessages) + pull-to-refresh + SkeletonRows; "الجلسة القادمة" still static, no backend field (~1050 tok)
- `homework_view.tsx` — ParentHomeworkView, real data (useChildHomework) + pull-to-refresh + SkeletonRows (~990 tok)
- `messages.tsx` — ParentMessages, now wired to useChildMessages (was mock MSGS) + pull-to-refresh + SkeletonRows (~740 tok)
- `recordings.tsx` — ParentRecordings, real data (useChildRecordings) + pull-to-refresh + SkeletonRows (~790 tok)
- `timeline.tsx` — TL, still hardcoded hijri-milestone mock — no backend juz/milestone-history endpoint exists to wire it to (~894 tok)
...
- `evaluate.tsx` — Scores are keyed by the active plan's rubric, so categories are not known (~3996 tok)
...
- `EvaluationRoster.tsx` — Scores are keyed by the active plan's rubric — not known at compile time. (~9349 tok)
...
- `MoreSheet.tsx` — Accent per icon rather than per position, so an item keeps the same colour no (~3493 tok)
...
- `evaluationRubric.ts` — The daily grading split is per plan now (`QuranPlan.gradeRubric`), not a (~861 tok)
...
- `genderScope.ts` — Admin-wide "which masajid gender am I looking at" filter, selected from (~442 tok)
- `portals.ts` — Exports PORTALS, PORTAL_ROUTES (~1330 tok)
...
- `quranPlan.ts` — One line of a plan's daily grading rubric — what is graded, out of how many. (~3819 tok)
...
- `portalStore.ts` — Whether the user opted into Face ID/Touch ID re-auth (Account Settings). Persisted. (~2580 tok)
...
- `_check_counts.ts` — Declares main (~307 tok)
- `_check_links.ts` — Declares main (~279 tok)
- `_check_users.ts` — Declares main (~282 tok)
- `_cleanup_orphans.ts` — Declares main (~418 tok)
...
- `admin.controller.ts` — Zod schemas: updateParentSchema, createParentSchema. getParents drops null (deleted-student) children (~1530 tok)
...
- `evaluation.controller.ts` — The rubric is no longer platform-wide — each plan carries its own (~2527 tok)
...
- `stats.controller.ts` — Exports getDashboardStats (~1047 tok)
- `student-plan-progress.controller.ts` — Returns the student's effective schedule: the shared plan's own schedule (~4651 tok)
- `student.controller.ts` — Empty string is normalised to undefined so a blank field clears rather. deleteStudent cascades ParentStudent link + orphaned User; ?withParent=true also drops the parent User (~2320 tok)
...
- `quran-plan.routes.ts` — API routes: GET, POST, PUT, DELETE (12 endpoints) (~632 tok)
...
## quran-hifz-server/src/validators/

- `context.ts` — API routes: GET (3 endpoints) (~272 tok)
...
- `quran-plans.ts` — sessionStorage key used to hand off "open the plan form" from wherever a (~2623 tok)
...
- `TrackStudentsPanel.tsx` — Track roster management — transfer-only, since a student's track is (~2003 tok)
...
- `PortalContext.tsx` — TopbarValueContext (~1403 tok)
...
- `evaluationRubric.ts` — The daily grading split is per plan now (`QuranPlan.gradeRubric`), not a (~817 tok)
- `genderScope.ts` — Admin-wide "which masajid gender am I looking at" filter, selected from (~523 tok)
...
- `AdminDashboard.tsx` — PageLoading (~1359 tok)
- `AdminHalqat.tsx` — OVERLAY — renders modal (~3688 tok)
- `AdminKpis.tsx` — RATING_TONE — renders table (~743 tok)
- `AdminMasajid.tsx` — OVERLAY — renders modal (~3180 tok)
- `AdminParents.tsx` — EMPTY_ADD — renders table (~5487 tok)
...
- `AdminStudents.tsx` — PATH_TONE — renders table. Delete dialog offers student-only vs student+parent when parentName is linked (~6040 tok)
- `AdminTeachers.tsx` — EMPTY_FORM — renders form, table, modal (~4749 tok)
- `AdminTrackForm.tsx` — Full-page create/edit form for a track (was a popup modal) — lets the (~4822 tok)
- `AdminTracks.tsx` — surahName — renders modal (~6365 tok)

## quran-hifz/src/quran/pages/common/

- `AccountSettings.tsx` — profileSchema — renders form (~2016 tok)
- `AccountSettings.tsx` — profileSchema — renders form (~1966 tok)
...
- `TeacherPlanForm.tsx` — One type's track in the form: its own days and its own range. The plan's (~14212 tok)
- `TeacherPlans.tsx` — surahName (~4522 tok)
